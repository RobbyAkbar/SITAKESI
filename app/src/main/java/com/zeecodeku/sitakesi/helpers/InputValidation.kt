package com.zeecodeku.sitakesi.helpers
 ...
 */
    ...
    /**
     * method to check InputEditText has minimum value.
     *
     * @param textInputEditText
     * @param textInputLayout
     * @param minimum
     * @return
     */
    fun isInputTextMinimum(textInputEditText: TextInputEditText, textInputLayout: TextInputLayout, minimum: Int): Boolean {
        val value = textInputEditText.text.toString().trim()
        if (value.length<minimum){
            textInputLayout.error = "at least $minimum characters or more"
            hideKeyboardFrom(textInputEditText)
            return false
        } else {
            textInputLayout.isErrorEnabled = false
